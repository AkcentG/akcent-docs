---
title: Reference Library
description: A quick-reference guide to common Source (Wikitext) syntax, formatting, and examples used on the Restaurant Tycoon 3 Wiki.
---

# 📖 Reference Library

The **Reference Library** is your quick-reference guide while editing the **Restaurant Tycoon 3 Wiki**.

Instead of searching through multiple websites, you can use this page as a handy cheat sheet for common Source (Wikitext) syntax, formatting, templates, and examples.

![Wikitext formatting reference](/screenshots/formatting-guide.png)

*A cheat sheet of Wiki Text (Formatting Guide)*

---

## Quick Reference

Looking for something specific? Jump straight to the section you need.

### Most commonly used

- 🖼️ **[Images](#images)** — Add and format images
- 📊 **[Tables](#tables)** — Create and format tables
- 📑 **[Tabbers](#tabbers)** — Organize information into tabs

### Other useful tools

- 🌐 **[Roblox API](#roblox-api)** — Retrieve information from Roblox
- ↪️ **[Redirects](#redirects)** — Redirect one wiki page to another

---

## ✍️ Text Formatting

| Syntax | Result |
|----------|--------|
| `'''Bold'''` | **Bold** |
| `''Italic''` | *Italic* |
| `'''''Bold Italic'''''` | ***Bold Italic*** |

---

## 📑 Headings

Use headings to organise your articles into sections.

```wiki
== Main Heading ==

=== Subheading ===

==== Smaller Heading ====
```

---

## 🔗 Internal Links

Link to another page on the wiki.

```wiki
[[Restaurant]]
```

Display custom text.

```wiki
[[Restaurant|Restaurant Guide]]
```

---

## 🌐 External Links

Link to websites outside the wiki.

```wiki
[https://example.com Example Website]
```

---

## 📋 Lists

### Bullet List

```wiki
* Item One
* Item Two
* Item Three
```

### Numbered List

```wiki
## First Item
## Second Item
## Third Item
```

---

<a id="images"></a>

## 🖼 Images

Basic image.

```wiki
[[File:Example.png]]
```

Thumbnail with caption.

```wiki
[[File:Example.png|thumb|Example Caption]]
```

Custom image width.

```wiki
[[File:Example.png|300px]]
```

---

## 📦 Infobox Example

Infoboxes are commonly used to present important information about a page in a structured format.

![Infobox example](/screenshots/infobox-example.png)

---

<a id="tables"></a>

## 📊 Tables

Basic wiki table.

```wiki
{| class="wikitable"
! Name
! Category
|-
| Burger
| Food
|-
| Pizza
| Food
|}
```

---

## 📦 Templates

Basic template.

```wiki
{{TemplateName}}
```

Template with parameters.

```wiki
{{TemplateName
|name=Example
|image=Example.png
}}
```

---

<a id="roblox-api"></a>

## 🌐 Roblox API

The Restaurant Tycoon 3 Wiki has access to a `#robloxAPI` parser function that can retrieve certain information from Roblox.

For example, it can be used to retrieve a game's icon:

```wiki
{{#robloxAPI: gameIconUrl | 119048529960596 | 256x256 }}

```
<a id="redirects"></a>

## 🔄 Redirects

Redirect a page to another page.

```wiki
#REDIRECT [[Target Page]]
```

Example.

```wiki
#REDIRECT [[Build Mode]]
```

---

<a id="tabbers"></a>

## 📑 Tabbers

Create multiple tabs within a page.

```wiki
<tabber>

Overview=
Content goes here.

|-|

Gallery=
More content goes here.

|-|

Trivia=
Additional content.

</tabber>
```

---

## 💬 Comments

Comments are only visible in the source editor.

```wiki
<!-- This is a comment -->
```

---

## 📂 Categories

Place a page into a category.

```wiki
[[Category:Restaurants]]
```

A page can belong to multiple categories.

```wiki
[[Category:Restaurants]]
[[Category:Gameplay]]
```

---

## ⭐ Common Editing Workflow

A typical editing workflow looks like this:

1. Open the page.
2. Click **Edit Source**.
3. Make your changes.
4. Preview your work.
5. Check formatting and links.
6. Write an edit summary.
7. Save your changes.

Following this process helps reduce mistakes and keeps edits organised.

---

## ✅ Editing Checklist

Before saving your edit, ask yourself:

- Have I previewed my changes?
- Are all links working?
- Is the formatting consistent?
- Are templates displaying correctly?
- Did I write an edit summary?
- Have I checked for spelling or grammar mistakes?

A quick review can prevent common editing errors.

---

## 📚 Related Pages

Need more than a quick reference?

| Page | Purpose |
|------|---------|
| **Quick Start** | Learn the basics of editing. |
| **Tips & Tricks** | Improve your editing workflow. |
| **Questions & Answers** | Find solutions to common editing problems. |
| **Resource Directory** | Browse official documentation and useful websites. |

---

## 💡 Keep This Page Bookmarked

As you continue editing, you'll likely return to this page often. It's designed to be a fast, practical reference that grows alongside Akcent Docs.

If you discover useful syntax, templates, or editing techniques, consider adding them here to help future editors.
